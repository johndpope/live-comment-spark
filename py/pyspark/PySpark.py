...
def config():
    ...
    global corpus_filename, K, alpha, beta, max_iter, \
        seed, checkin_point_interval, optimizer
    ...
    os.environ['PYTHONPATH'] = conf.get(SECTION, 'PYTHONPATH')
...
if __name__ == '__main__':
    ...
    data = sc.textFile(corpus_filename) \
                       .map(lambda line:
                            Vectors.dense(
    ...
    print(topics)
    # rm
    topics_all = lda_model.describeTopics()
    print(topics_all[0],topics_all[1],np.array(topics_all[0]).size)
